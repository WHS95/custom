...
export function GalleryClient({
  ...
  const openImageModal = (review: Review, index = 0) => {
  ...
  const goToNextImage = () => {
    if (
      selectedReview &&
      currentImageIndex < selectedReview.images.length - 1
    ) {
      setCurrentImageIndex((prev) => prev + 1);
    }
  };
        ...
        <section className='mb-12'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
            {featuredReviews.map((review) => (
              <Card
                key={review.id}
                className='overflow-hidden border-2 border-yellow-200 bg-yellow-50/30'
              >
      ...
      {selectedReview && selectedReview.images.length > 0 && (
        ...
        >
          ...
          <img
            ...
            alt={
              selectedReview.images[currentImageIndex].caption || "후기 이미지"
            }
