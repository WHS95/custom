...
export function AdminDashboardClient({
  ...
  const { config, isLoading, isSaving, error, updateConfig, refreshConfig } =
    useStudioConfig();

  const [localShippingCost, setLocalShippingCost] = useState<number | null>(
    null,
  );
  const [localShippingFreeThreshold, setLocalShippingFreeThreshold] = useState<
    number | null
  >(null);
    ...
    <div className='container mx-auto py-8'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8'>
        ...
        >
          <CardHeader>
            ...
            <CardDescription>
              크루 멤버 인증 요청 승인/거절 관리
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className='flex items-center text-amber-600 font-medium'>
              크루 승인으로 이동 <ArrowRight className='ml-2 h-4 w-4' />
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className='mt-8 p-4 bg-gray-50 rounded-lg border'>
        ...
        <ul className='text-sm text-gray-600 space-y-1'>
          <li>
            • <strong>상품 가격</strong>: 상품 관리 → 상품 수정에서 설정
          </li>
          <li>
            • <strong>상품 이미지</strong>: 상품 관리 → 이미지 버튼 →
            색상별/뷰별 업로드
          </li>
          <li>
            • <strong>인쇄 영역</strong>: 상품 관리 → 이미지 버튼 → 인쇄 영역
            설정 탭
          </li>
          <li>
            • <strong>색상/사이즈</strong>: 상품 관리 → 상품 수정에서 설정
          </li>
        </ul>
      </div>
    </div>
